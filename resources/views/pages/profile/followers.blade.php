<x-layouts.app :title="($user->profile?->display_name ?? $user->name) . ' · Pengikut'">
...
<div class="mx-auto" style="max-width:600px;">
    ...
    @forelse($followers as $follower)
        @include('components.user-row', ['user' => $follower, 'authUser' => $authUser])
    @empty
        <div class="text-center py-5 text-secondary">
            <i class="ri-user-line d-block fs-1 mb-3 opacity-50"></i>
            <p class="mb-0">Belum ada pengikut.</p>
        </div>
    @endforelse
    ...
    <div class="d-flex justify-content-center gap-2 p-3">
        @if(!$followers->onFirstPage())
            <a href="{{ $followers->previousPageUrl() }}" class="btn btn-outline-secondary btn-sm rounded-pill">‹</a>
        @endif
        @if($followers->hasMorePages())
            <a href="{{ $followers->nextPageUrl() }}" class="btn btn-outline-secondary btn-sm rounded-pill">›</a>
        @endif
    </div>
    @endif
</div>
